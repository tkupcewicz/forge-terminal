...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let mut order: Vec<&'static str> = vec!["Forge", "Shell", "Edit", "View", "Window"];
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    ...
                    } else if cmd.menubar[0] == "Forge" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("Forge {}", config::wezterm_version()),
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        QuitApplication => CommandDef {
            brief: "Quit Forge".into(),
            doc: "Quits Forge".into(),
        ...
        ClaudeAccept => CommandDef {
            ...
            keys: vec![(Modifiers::CTRL, "y".into())],
            args: &[],
            menubar: &["Claude"],
            icon: Some("md_check"),
        },
        ...
        ClaudeVerboseMode => CommandDef {
            ...
            keys: vec![(Modifiers::CTRL.union(Modifiers::SHIFT), "v".into())],
            args: &[],
            menubar: &["Claude"],
            icon: Some("md_expand"),
        },
